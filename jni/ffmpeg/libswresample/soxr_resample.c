    ...
    double cutoff, enum AVSampleFormat format, enum SwrFilterType filter_type, int kaiser_beta, double precision, int cheby)
{
    ...
    struct soxr_io_spec_t io_spec = soxr_io_spec(type, type);

    struct soxr_quality_spec_t q_spec = soxr_quality_spec((int) ((precision - 2) / 4), (SOXR_HI_PREC_CLOCK | SOXR_ROLLOFF_NONE) * !!cheby);
...
{
    soxr_process((struct soxr_t) s->resample, NULL, 0, NULL, NULL, 0, NULL);
    return 0;
}
...
{
    size_t idone, odone;
    char const * error = soxr_set_error((struct soxr_t) c, soxr_set_num_channels((struct soxr_t) c, src->ch_count));
    if (!error)
        error = soxr_process((struct soxr_t) c, src->ch, (size_t) src_size,
...
{
    double delay_s = soxr_delay((struct soxr) s->resample) / s->out_sample_rate;
